...
`npm run fetch-engagement`는 `rlaguswls13/giscus-blog` 저장소의 공개 Discussions를 읽어 `src/data/indexes/engagement.json`에 저장합니다. 네트워크 요청이 실패하면 마지막 캐시를 그대로 사용하므로 정적 빌드는 계속 진행됩니다.
