...
| `BASE_PATH=ROOT` | 운영 빌드를 도메인 루트에 배포 (`ROOT`가 아니면 지정값 또는 `/blog`) |
...
GitHub Actions는 Node.js 20에서 의존성을 설치하고 Notion 콘텐츠를 동기화한 다음 `out/`을 GitHub Pages artifact로 배포합니다. 운영 `basePath`는 `USE_ROOT_BASE_PATH` 저장소 변수에 따라 루트 또는 저장소명 경로로 결정됩니다.

추가 운영 방법은 [`docs/GUIDE.md`](docs/GUIDE.md), 기술 배경은 [`docs/TECH_STACK.md`](docs/TECH_STACK.md)를 참고하세요.
