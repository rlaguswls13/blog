...
date: "2026.05.26"
tags: ["TechStudy", "Docker", "Network", "Port"]
...
*   **사용자 정의 Bridge 네트워크 생성:** 
    ...
    docker run --network my-net --name backend my_app
    docker run --network my-net --name database my_db
    ```
    *   **강점:** 같은 커스텀 브리지 네트워크에 묶인 컨테이너들은 IP 주소를 외울 필요 없이, 컨테이너 이름(DNS)으로 서로 통신할 수 있습니다. (예: `http://database:3306`)
