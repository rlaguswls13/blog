...
const S = {
  ...
  tabBorder: {
    borderBottom: '1px solid var(--diagram-card-border)',
  } as React.CSSProperties,
...
export function StorageArchitectureDiagram() {
  const [activeTab, setActiveTab] = useState<"physical" | "network">("physical");
  ...
  const tabClass = (active: boolean) =>
    `py-3 px-6 font-bold text-base transition-all duration-200 border-b-2 ${active
      ? 'border-[#6366f1]'
      : 'border-transparent hover:opacity-80'
    }`;
  const tabColor = (active: boolean): React.CSSProperties =>
    active ? { color: '#6366f1' } : { color: 'var(--diagram-text-secondary)' };
  ...
  return (
    <>
      ...
      <div className="w-full mt-8 font-sans" style={S.heading}>
        <div className="max-w-7xl mx-auto">
          {/* Tabs */}
          <div className="flex mb-8 overflow-x-auto whitespace-nowrap" style={S.tabBorder}>
            <button onClick={() => setActiveTab("physical")} className={tabClass(activeTab === "physical")} style={tabColor(activeTab === "physical")}>
              물리적 저장 매체 비교 (NVMe SSD vs SATA SSD vs HDD)
            </button>
            <button onClick={() => setActiveTab("network")} className={tabClass(activeTab === "network")} style={tabColor(activeTab === "network")}>
              네트워크 아키텍처 비교 (DAS vs NAS vs SAN)
            </button>
          </div>

          {/* ========== Tab 1: Physical Media ========== */}
          {activeTab === "physical" && (
                  ...
                  <div>
                    ...
                    <div className="rounded-2xl p-4 flex flex-col items-center justify-center h-48 relative mb-6" style={S.innerBox}>
                      <svg className="h-32 w-52" viewBox="0 0 200 120" fill="none">
                        ...
                        <rect x="50" y="49" width="22" height="22" rx="1" fill="#475569" stroke="#64748b" strokeWidth="1.5" />
          ...
          {activeTab === "network" && (
            <section className="space-y-12 animate-fade-in">
              ...
              <div className="rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6" style={{ ...S.card, background: ACCENT.indigo.light }}>
                <div className="space-y-1">
                  <h2 className="text-lg font-bold" style={S.heading}>사용 목적에 따른 네트워크 스토리지 분류</h2>
